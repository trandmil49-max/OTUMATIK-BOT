...
from core.models import Report, ReportType, Signal, SignalDirection, Trade, TradeStatus
...
class ReportingEngine:
    ...
    def _build_content(self, trades: list[Trade], signals: list[Signal]) -> dict[str, Any]:
        ...
        trailing_exits = [t for t in trades if t.status == TradeStatus.TRAILING_STOP_EXIT]
        other = [
            t for t in trades
        ...
        short_signals = sum(1 for s in signals if s.direction == SignalDirection.SHORT)
        # Signals from this period that haven't produced a closed trade yet --
        # still WAITING to activate, or ACTIVE and not yet closed. An
        # approximation (signals generated minus trades closed), not a
        # separate status join: exactly right for a report window that
        # doesn't span a signal's entire lifecycle edge case, and never
        # negative regardless.
        still_open = max(0, len(signals) - len(trades))
        confidence_values = [s.confidence_score for s in signals if s.confidence_score is not None]

        best_trade = max(trades, key=lambda t: t.realized_pnl_percent, default=None) if pnl_values else None
        worst_trade = min(trades, key=lambda t: t.realized_pnl_percent, default=None) if pnl_values else None
        ...
        return {
            ...
            "best_trade_pnl_percent": round(best_trade.realized_pnl_percent, 2) if best_trade else None,
            "best_trade_symbol": best_trade.symbol if best_trade else None,
            "best_trade_direction": best_trade.direction.value if best_trade else None,
            "worst_trade_pnl_percent": round(worst_trade.realized_pnl_percent, 2) if worst_trade else None,
            "worst_trade_symbol": worst_trade.symbol if worst_trade else None,
            "worst_trade_direction": worst_trade.direction.value if worst_trade else None,
        }
