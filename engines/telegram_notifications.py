...
class TelegramNotificationEngine:
    ...
    def __init__(
        ...
        # Both optional, both for notify_new_signal()'s confirmation
        # checklist / "BTC Status" line only (see that method's
        # docstring) -- repositories, not another engine instance, the
        # same dependency shape ReportingEngine already uses, so this
        # module never needs a whole BitcoinIntelligenceEngine (which in
        # turn would need a live Binance client) just to read back
        # already-persisted data. `None` means that section is silently
        # omitted from the message, not an error -- graceful degradation,
        # same posture as a missing macro_client inside analyze().
    ...
    # Categories from ConfidenceEngine.CATEGORY_MAX_POINTS that get a
    # checklist line of their own. "bitcoin" is deliberately excluded --
    # its own finer-grained lines (BTC Trend/DXY/BTC Dominance/USDT
    # Dominance) come from BitcoinIntelligenceEngine.confirmed_macro_reasons()
    # instead, via the actual raw trend classifications rather than one
    # opaque blended category score, so a line here is never a coarser,
    # confusingly-overlapping duplicate of one already coming from there.
    _CHECKLIST_CATEGORY_LABELS: dict[str, str] = {
        "trend": "Trend Confirmed",
        "structure": "Structure Confirmed",
        "risk": "Risk Confirmed",
        "coin_trust": "Coin Trust Confirmed",
        "market_health": "Market Health Confirmed",
        "smart_money": "Smart Money Confirmed",
    }
    # A category must clear this fraction of its own max weight to count
    # as "Confirmed" here. Not from any spec -- sinyal_kanali_2's
    # checklist was built from ~20 flat pass/fail hard filters with no
    # equivalent "fraction of max" concept, so this threshold has no
    # reference value to port. 0.7 was chosen as a reasonably strict bar
    # (clearly, not just marginally, positive) consistent with this
    # project's general confidence-grading conventions; tune via
    # discussion if it under- or over-reports in practice.
    _CHECKLIST_CONFIRMATION_RATIO = 0.7

    def _confirmation_checklist(self, signal: Signal) -> list[str]:
        lines: list[str] = []
        if self._signal_repository is not None and signal.id is not None:
            breakdown = self._signal_repository.get_score_breakdown(signal.id)
            max_points = ConfidenceEngine.CATEGORY_MAX_POINTS
            for component in breakdown:
                label = self._CHECKLIST_CATEGORY_LABELS.get(component.category)
                category_max = max_points.get(component.category)
                if label is None or not category_max:
                    continue
                if component.points / category_max >= self._CHECKLIST_CONFIRMATION_RATIO:
                    lines.append(f"✅ {label}")
        if self._btc_repository is not None:
            btc_snapshot = self._btc_repository.get_latest()
            if btc_snapshot is not None:
                macro_reasons = BitcoinIntelligenceEngine.confirmed_macro_reasons(
                    signal.direction, btc_snapshot, symbol=signal.symbol
                )
                lines.extend(f"✅ {reason}" for reason in macro_reasons)
        return lines
    ...
    async def notify_new_signal(self, signal: Signal) -> bool:
        """
        SRS Part 11 TELEGRAM SIGNAL MESSAGE.

        Wording matches the platform owner's exact reference format
        (screenshots of `sinyal_kanali_2`'s live output, provided
        directly -- "tam bunun gibi olacak"), not this module's earlier
        "premium redesign" text -- a later explicit, concrete spec
        supersedes an earlier undirected one.

        Confirmation checklist and "BTC Status" line (both optional --
        silently omitted if `signal_repository`/`btc_repository` weren't
        injected, or if the underlying data isn't there): granularity is
        NOT a 1:1 match with sinyal_kanali_2's reference. That reference
        was built from ~20 flat, independently-named pass/fail hard
        filters; this platform's `ConfidenceEngine` blends into 7 named
        CATEGORIES instead, several of which (Trend, Structure, Risk,
        Coin Trust, Market Health, Smart Money) are each already a single
        coherent concept and map onto one checklist line apiece cleanly.
        The exception is "Bitcoin", which blends BTC's own trend
        alignment together with the DXY/BTC-dominance/USDT-dominance
        macro nudge (see `BitcoinIntelligenceEngine._macro_adjustment`)
        into one opaque score -- reporting that as a single "Bitcoin
        Confirmed" line would lose exactly the granularity the reference
        format has (separate DXY/BTC Dominance/USDT Dominance lines), so
        those come from `confirmed_macro_reasons()` instead, computed
        from the same raw trend classifications `_macro_adjustment`
        scores by rather than from the blended total. `sinyal_kanali_2`'s
        remaining ~14 filters (US equities, individual liquidity-sweep/
        SR-zone touches, etc.) have no equivalent signal anywhere in this
        codebase and are not shown -- inventing a line for a check that
        was never actually run would violate SRS Part 9's "every decision
        must be explainable after the fact."
        """
        ...
        lines = [
            "🚨 <b>NEW SIGNAL</b>",
            "",
            f"{direction_icon} <b>{direction_label}</b>",
            "",
            f"🪙 Coin: <b>{escape_html(signal.symbol)}</b>",
            f"⭐ Confidence: <b>{signal.confidence_score:.0f}/100</b>",
            f"💰 Entry: <code>{_fmt_price(signal.entry_price)}</code>",
            f"🛑 Stop Loss: <code>{_fmt_price(signal.stop_loss)}</code>",
            "",
            f"🎯 TP1: <code>{_fmt_price(signal.take_profit_1)}</code>",
            "",
            f"⚡ Leverage: <b>{signal.leverage}x</b>",
            f"📊 Risk/Reward: <b>{signal.risk_reward_ratio:.2f}</b>",
        ]
        ...
        if self._btc_repository is not None:
            btc_snapshot = self._btc_repository.get_latest()
            if btc_snapshot is not None:
                btc_status = BitcoinIntelligenceEngine.classify_status(
                    btc_snapshot.health_score, self._config.bitcoin.health_threshold
                )
                lines.append(f"₿ BTC Status: <b>{btc_status}</b>")
        checklist = self._confirmation_checklist(signal)
        lines.append("")
        if checklist:
            lines.extend(checklist)
        else:
            # sinyal_kanali_2's own format_reasons() falls back to this exact
            # line when its allowlist matches nothing either -- an empty
            # checklist isn't a special/error case, just means nothing
            # cleared this message's bar this time.
            lines.append("✅ Multi-layer validation passed")
        lines.append("")
        lines.append(f"🕒 Signal Time: {self._fmt_time(signal.created_at)}")
    ...
    def _format_report_body(self, report: Report) -> str:
        ...
        lines = [f"📊 <b>{title}</b>", f"🗓 Dönem: {period}"]

        stats_lines = self._format_report_stats(report.content)
        if stats_lines:
            lines.append("")
            lines.extend(stats_lines)

        if report.turkish_analysis:
            lines.append("")
            lines.append(escape_html(report.turkish_analysis))
        if report.turkish_recommendations:
            lines.append("")
            lines.append("<b>Öneriler:</b>")
            lines.append(escape_html(report.turkish_recommendations))
        if report.overall_grade:
            lines.append("")
            lines.append(f"🏁 Genel Not: <b>{escape_html(report.overall_grade)}</b>")
        return "\n".join(lines)

    @staticmethod
    def _format_report_stats(content: dict) -> list[str]:
        """
        Surfaces `Report.content`'s already-computed statistics (SRS Part
        13's daily/weekly/monthly numbers -- `engines/reporting.py` never
        needed to change for this, the numbers were always there, just
        not shown with their own labeled line before) using the emoji
        the platform owner specified. Every field is optional -- a report
        built from zero closed trades has a `content` dict without
        PNL/win-rate keys at all, so each line is only added when its
        value is actually present, the same "omit, don't fabricate"
        pattern `_format_report_body()` already used for recommendations
        and an overall grade.
        """
        lines: list[str] = []
        total_pnl = content.get("total_pnl_percent")
        if total_pnl is not None:
            lines.append(f"💵 Toplam PNL: <code>{_fmt_signed_pct(total_pnl)}</code>")
        win_rate = content.get("win_rate_percent")
        if win_rate is not None:
            lines.append(f"📈 Kazanma Oranı: <code>{_fmt_pct(win_rate)}</code>")
        wins, losses = content.get("wins"), content.get("losses")
        if wins is not None and losses is not None:
            lines.append(f"✅ Kâr: <code>{wins}</code> · ❌ Zarar: <code>{losses}</code>")
        signals_generated = content.get("signals_generated")
        if signals_generated is not None:
            lines.append(f"🟢 Üretilen Sinyal: <code>{signals_generated}</code>")
        best_trade = content.get("best_trade_pnl_percent")
        if best_trade is not None:
            lines.append(f"🏅 En İyi İşlem: <code>{_fmt_signed_pct(best_trade)}</code>")
        worst_trade = content.get("worst_trade_pnl_percent")
        if worst_trade is not None:
            lines.append(f"⚠ En Kötü İşlem: <code>{_fmt_signed_pct(worst_trade)}</code>")
        return lines
